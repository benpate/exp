...
// TestAndOrShortcuts confirms that each Or* shortcut wraps the AndExpression in
// an OrExpression with the new predicate as the second entry.
func TestAndOrShortcuts(t *testing.T) {
	...
	run := func(name string, got Expression, operator string) {
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(t, base, or[0]) // original AndExpression is preserved as the first entry
